...
char
yVAR_char               (char *a_test, uchar a_expect, uchar a_actual)
{
   /*---(design notes)--------------------------------------------------------#
...
yVAR_integer            (char *a_test, llong a_expect, llong a_actual)
{
   ...
   /*---(poximity tests)-----------------*/
      ...
      if (strchr ("0123456789", a_test [6]) == NULL)  return -('P');
      ...
      if (strchr ("0123456789", a_test [8]) == NULL)  return -('R');
      a = a_test [8] - '0';
      rc = 'r';
      ...
      double    a_actual)        /*  actual result                            */
{
   ...
   if  (strcmp(a_test, "r_equal")    == 0) {
      ...
      rc = -('s');
      snprintf (yVAR_expstr, 15, "%9.7lf", a_expect);
      snprintf (yVAR_actstr, 15, "%9.7lf", a_actual);
      if (strcmp(yVAR_expstr, yVAR_actstr) == 0)    rc *= -1;
   } else {
      rc = '¢';
   }
